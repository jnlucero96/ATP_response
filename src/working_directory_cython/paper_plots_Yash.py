...
def plot_power_efficiency_Ecouple(path):  # plot power and efficiency vs coupling strength
    Ecouple_array_tot = array(
        [2.0, 2.83, 4.0, 5.66, 8.0, 10.0, 11.31, 12.0, 14.0, 16.0, 18.0, 20.0, 22.0, 24.0, 32.0,
         45.25, 64.0, 90.51, 128.0])

    output_file_name = (
            path + "/plots/" + "P_ATP_eff_Ecouple_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_phi_{4}" + "_.pdf")
    ...
    for ii, Ecouple in enumerate(Ecouple_array_tot):
        input_file_name = (path + "/plotting_data/" + "flux_power_efficiency_"
                           + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
        try:
            data_array = loadtxt(
    ...
    for ii, Ecouple in enumerate(Ecouple_array_tot):
        input_file_name = (
                path + "/plotting_data/flux_power_efficiency_"
                + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
        try:
            data_array = loadtxt(
...
def plot_power_Ecouple_grid(path):  # grid of plots of the flux as a function of the phase offset
    ...
    output_file_name = (path +"/plots/"+ "P_ATP_Ecouple_grid_" + "E0_{0}_E1_{1}_n1_{2}_n2_{3}" + "_.pdf")
...
def plot_power_efficiency_phi(path): # plot power and efficiency as a function of the coupling strength
    output_file_name = (
                path + "/plots/"+ "power_efficiency_phi_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_Ecouple_{4}" + "_log_.pdf")
...
def plot_power_phi_single(path):  # plot of the power as a function of the phase offset
    colorlst = ['C2', 'C3', 'C1', 'C4']
    markerlst = ['D', 's', 'o', 'v']
    Ecouple_array = array([2.0, 8.0, 16.0, 32.0])

    output_file_name = (path + "/plots/"
...
def plot_nn_power_efficiency_Ecouple(path):  # plot power and efficiency as a function of the coupling strength
    markerlst = ['D', 's', 'o', 'v', 'x']
    ...
    output_file_name = (
            path + "/plots/" + "power_nn_efficiency_Ecouple_plot_more_"
...
def plot_nn_power_efficiency_phi(path):  # plot power and efficiency as a function of the coupling strength
    ...
    output_file_name = (
            path + "/plots/"+ "power_efficiency_phi_vary_n_"
...
def plot_n0_power_efficiency_Ecouple(path):  # plot power and efficiency as a function of the coupling strength
    markerlst = ['D', 's', 'o', 'v', 'x']
    color_lst = ['C2', 'C3', 'C1', 'C4', 'C6']
    Ecouple_array_tot = array(
        [8.0, 11.31, 16.0, 22.63, 32.0, 45.25, 64.0, 90.51, 128.0])
    output_file_name = (
                path + "/plots/" + "/power_efficiency_Ecouple_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n2_{4}" + "_log_.pdf")
    f, axarr = plt.subplots(2, 2, sharex='col', sharey='row', figsize=(6, 6),
                            gridspec_kw={'width_ratios': [10, 1], 'height_ratios': [2, 1]})
...
if __name__ == "__main__":
    #path = "/Users/Emma/sfuvault/SivakGroup/Emma/ATPsynthase/FokkerPlanck_2D_full/" + \
               #  "prediction/fokker_planck/working_directory_cython/"
    path = os.getcwd()  
    
    flux_power_efficiency(path)
    plot_power_efficiency_Ecouple(path)
    plot_power_Ecouple_grid(path)
    plot_power_efficiency_phi(path)
    plot_power_phi_single(path)
    plot_nn_power_efficiency_Ecouple(path)
    plot_nn_power_efficiency_phi(path)
    plot_n0_power_efficiency_Ecouple(path)