...
info=open('info_naive.dat','w')
plots=False
if plots==True:
	print "yes"
	import matplotlib.pyplot as plt
	os.makedirs('Probs_naive')
...
while (time<cycles*Period+dt and SSS_cycle_counter<3):
	...
	trap_rad=TrapMin(time)
	trap_deg=trap_rad*360/(2*pi)
	c=trap_rad
	...
	# 	print time
	# 	plt.savefig('k'+str(k)+'P'+str(Period)+'/Naive/probs/t='+str(time)+'.png')
	# 	plt.close()
	# 	print_counter=0
	...
	if plots==True:
		if print_counter==print_frequency:
			if SSS_cycle_counter==2:
				...
				plt.xticks([0,2*pi/3,4*pi/3])
				print time
				plt.savefig('Probs_naive/t='+str(time)+'.png')
				plt.close()
			print_counter=0
