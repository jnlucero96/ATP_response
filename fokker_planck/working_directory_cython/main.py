...
from fpe import launchpad_coupled, launchpad_flows
...
def get_params():
    ...
    # Choices: coupled, flows (nostalgia, fluxes)
    mode = "flows"
...
def save_data_flows(
    ...
    with open(summary_total_path, 'a') as sfile:
        ...
        sfile.write(
                ...
                + "{0:.15e}".format(mean_space_flux_y[0]) + '\t' #15
                + "{0:.15e}".format(mean_space_flux_y[1]) + '\t' #16
                + "{0:.15e}".format(mean_space_flux_y.mean()) + '\t' #17
                + "{0:.15e}".format(mean_space_time_flux_x[0]) + '\t' #18
                + "{0:.15e}".format(mean_space_time_flux_x[1]) + '\t' # 19
                + "{0:.15e}".format(mean_space_time_flux_x.mean()) + '\n' #20
                + "{0:.15e}".format(mean_space_time_flux_y[0]) + '\t' #18
                + "{0:.15e}".format(mean_space_time_flux_y[1]) + '\t' # 19
...
def main():
    ...
    if mode.lower() == "coupled":
        ...
        t0 = time()
        work, heat, nostalgia = launchpad_flows(
