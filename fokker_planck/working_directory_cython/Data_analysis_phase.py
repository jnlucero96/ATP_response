...
psi_ratio = array([0.5, 0.25, 0.125])
...
Ecouple_array_tot = array([0.0, 1.41, 2.0, 2.83, 4.0, 5.66, 8.0, 11.31, 16.0, 22.63, 32.0, 45.25, 64.0, 90.51, 128.0])
# Ecouple_array_tot = array([0.0, 1.41, 2.0, 2.83, 4.0, 5.66, 8.0, 10.0, 11.31, 12.0, 14.0, 16.0, 18.0, 20.0, 22.0, 22.63, 24.0, 32.0, 45.25, 64.0, 90.51, 128.0])
...
ticklst=linspace(0, 2*math.pi/3, 7)
                ...
                for Ecouple in Ecouple_array:
                    for ii, phase_shift in enumerate(phase_array):
                        if phase_shift in phase_array_test:
                            ...
                            input_file_name = ("/Users/Emma/Documents/Data/ATPsynthase/Full-2D-FP/191221_morepoints" + "/reference_" + "E0_{0}_Ecouple_{1}_E1_{2}_psi1_{3}_psi2_{4}_n1_{5}_n2_{6}_phase_{7}" + "_outfile.dat")
                        ...
                        output_file_name = (target_dir + "191217_morepoints/" + "processed_data/flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n2_{4}_Ecouple_{5}" + "_outfile.dat")
    ...
    for i, psi_1 in enumerate(psi1_array):
        for j, psi_2 in enumerate(psi2_array):
            ...
            for ii, Ecouple in enumerate(Ecouple_array):
                input_file_name = (target_dir + "190624_Twopiweep_complete_set/" + "processed_data/flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
                try:
        ...
        for psi_2 in psi2_array:
            ...
            for ii, Ecouple in enumerate(Ecouple_array):
                ...
                try:
                    ...
                    ax.plot(phase_array[:7], power_y_array[:7], linestyle='-', marker='o', label=f'${Ecouple}$', markersize=8, linewidth=2)
                except OSError:
                    print('Missing file')      
            ...
            ax.plot(phase_array[:61], flux_array[:61], '-', color=plt.cm.cool(colorlist[3]), label=f'$\infty$', linewidth=2)
            ...
            leg = ax.legend(handles[::-1], labels[::-1], title=r'$\beta E_{\rm couple}$', fontsize=14, loc=[0.8, 0.1], frameon=False)
            leg_title = leg.get_title()
            leg_title.set_fontsize(14)
            # plt.xlabel(r'$\phi\ (\rm rad)$', fontsize=20)
            f.text(0.5, 0.02, r'$\phi\ (\rm rev)$', fontsize=20, ha='center')
            plt.ylabel(r'$\beta \mathcal{P}_{\rm ATP}\ (t_{\rm sim}^{-1})$', fontsize=20)
            plt.xticks(ticklst, label_lst)
            plt.yticks([-0.0002, 0, 0.0002, 0.0004])
...
def plot_power_efficiency_phi_single(target_dir):#plot power and efficiency as a function of the coupling strength
    output_file_name = (target_dir + "power_efficiency_phi_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_Ecouple_{4}" + "_log_.pdf")
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            ...
            input_file_name = ("/Users/Emma/sfuvault/SivakGroup/Emma/ATPsynthase/FokkerPlanck_2D_full/prediction/fokker_planck/working_directory_cython/190624_Twopisweep_complete_set/processed_data/" + "flux_zerobarrier_psi1_{0}_psi2_{1}_outfile.dat")
            ...
            axarr[0].plot(phase_array[:7], power_y[:7], 'o', color='C1', label='$2$', markersize=8)
            ...
            axarr[1].axhline((flux_y_array[4])/(flux_x_array[4]), color='C0', linewidth=2, label='$0$')
            ...
            axarr[1].plot(phase_array[:7], eff_array[:7]/(-psi_2/psi_1), 'o', color='C1', label='$2$', markersize=8)
            ...
            leg_title = leg.get_title()
            leg_title.set_fontsize(14)
            # axarr[1].set_xlabel(r'$\phi$', labelpad=12, fontsize=20)
...
def plot_efficiency_phi_single(target_dir):#plot of the efficiency as a function of phase offset
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            ...
            print('Figure for psi1=%f, psi2=%f' % (psi_1, psi_2))
            for ii, Ecouple in enumerate(Ecouple_array):
                input_file_name = (target_dir + "190530_Twopisweep/processed_data/" + "flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
                try:
...
def plot_flux_phi_grid(target_dir):#grid of plots of the flux as a function of the phase offset
    ...
    for i, psi_1 in enumerate(psi1_array):
        for j, psi_2 in enumerate(psi2_array):
            print('Figure for psi1=%f, psi2=%f' % (psi_1, psi_2))
            for ii, Ecouple in enumerate(Ecouple_array):
                input_file_name = (target_dir + "processed_data/flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
                try:
                    data_array = loadtxt(input_file_name.format(E0, E1, psi_1, psi_2, num_minima1, num_minima2, Ecouple), usecols=(0,1,2))
                    #print('Ecouple=%f'%Ecouple)
                    phase_array = data_array[:,0]
...
def plot_power_Ecouple_grid(target_dir):#grid of plots of the flux as a function of the phase offset
    ...
    for j, psi_1 in enumerate(psi1_array):
        for i, ratio in enumerate(psi_ratio):
        # for i, psi_2 in enumerate(psi2_array):
            ...
            input_file_name = (target_dir + "190624_Twopisweep_complete_set/processed_data/" + "flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}" + "_outfile.dat")
            try:
                data_array = loadtxt(input_file_name.format(E0, E1, psi_1, psi_2, num_minima1, num_minima2, 128.0), usecols=(3,4))
                power_x = array(data_array[0,0])
                power_y = -array(data_array[0,1])
            except OSError:
                print('Missing file flux')
                
            if (j == 1 and i == 0) or (j == 2 and i == 0) or (j == 2 and i == 1):
            ...
            axarr[i, j].set_xlim((1.5, 160))
            
            if i == 0:
                # axarr[i, j].set_yticks([-0.0001, 0, 0.0001, 0.0002])
                axarr[i, j].ticklabel_format(style='sci', axis='y', scilimits=(0,0))
            elif i == 1:
                # axarr[i, j].set_yticks([-0.0005, -0.00025, 0, 0.00025])
                axarr[i, j].ticklabel_format(style='sci', axis='y', scilimits=(0,0))
            # else:
                # axarr[i, j].set_yticks([-0.002, -0.001, 0])
                # axarr[i, j].set_yticklabels([r'$-20.$', r'$10.$', r'$0.$'])
            ...
            if j == psi1_array.size-1:
                axarr[i, j].set_ylabel(r'$%.3f$' % psi_ratio[i], labelpad=16, rotation=270, fontsize=14)
                axarr[i, j].yaxis.set_label_position('right')
            
            if i == 0:
                axarr[i,j].set_title(r'$%.0f$' % psi1_array[j], fontsize=14)
    ...
    f.text(0.95, 0.5, r'$\mu_{\rm ATP} / \mu_{\rm H^+}$', va='center', rotation=270, fontsize=20)
        ...
        for psi_2 in psi2_array:
            if abs(psi_1) >= abs(psi_2):
                ...
                for j, num in enumerate(min_array):
                    ...
                    flux_y_array=[]
                    for ii, Ecouple in enumerate(Ecouple_array):
                        input_file_name = (targt_dir + "190924_no_vary_n1_3/processed_data/" + "flux_power_efficiency_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n2_{4}_Ecouple_{5}" + "_outfile.dat")
                        try:
...
def plot_flux_contour(target_dir):#contourplot of the flux as a function as the states of the two subsystems
    ...
    output_file_name = (target_dir + "flux_theta0_theta1_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}_phase_{7}" + "_.pdf")
...
def plot_flux_space(target_dir):#plot of the integrated flux as a function of the position
    ...
    Ecouple=0.0
    input_file_name = ("/Users/Emma/Documents/Data/ATPsynthase/Full-2D-FP/190520_phaseoffset/" + "reference_" + "E0_{0}_Ecouple_{1}_E1_{2}_psi1_{3}_psi2_{4}_n1_{5}_n2_{6}_phase_{7}" + "_outfile.dat")
    output_file_name = (target_dir + "flux_theta_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_{6}_phase_{7}" + "_outfile.dat")
...
def plot_power_efficiency_Ecouple_single(target_dir):#plot power and efficiency as a function of the coupling strength
    output_file_name = (target_dir + "power_efficiency_Ecouple_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_phi_{4}" + "_log_.pdf")
    f,axarr=plt.subplots(2, 1, sharex='all', sharey='none', figsize=(6,8))
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            ...
            #         eff_array = append(eff_array, data_array[0])
            #     except OSError:
            #         print('Missing file efficiency extra points')
            # axarr[1].plot(Ecouple_array_extra, eff_array/(-psi_2/psi_1), 'o', color='C1', markersize=8)
...
def plot_power_Ecouple_scaled(target_dir):#plot of power as a function of coupling strength
    output_file_name = (target_dir + "power_scaled_Ecouple_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n_{4}" + "_log_.pdf")
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            if psi_1 > abs(psi_2):
                ...
                #Infinite coupling
                input_file_name = (target_dir + "190624_Twopisweep_complete_set/processed_data/" + "flux_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}_Ecouple_inf" + "_outfile.dat")
                try:
...
def plot_energy_flux(target_dir):#contourplot of the energy landscape with the 2D flux on top in the form of arrows
    ...
    input_file_name = ("/Users/Emma/Documents/Data/ATPsynthase/Full-2D-FP/190520_phaseoffset/" + "reference_" + "E0_{0}_Ecouple_{1}_E1_{2}_psi1_{3}_psi2_{4}_n1_{5}_n2_{6}_phase_{7}" + "_outfile.dat")
    output_file_name = (target_dir + "Energy_flux_" + "Ecouple_{0}_E0_{1}_E1_{2}_psi1_{3}_psi2_{4}_n1_{5}_n2_{6}_phase_{7}" + "_.pdf")
...
def plot_energy_flux_grid(target_dir):#grid of contourplots of the energy landscape with the 2D flux on top in the form of arrows
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            ...
            output_file_name = (target_dir + "190530_Twopisweep/Energy_flux_grid_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}" + "_.pdf")
...
def plot_prob_flux(target_dir):#contourplot of the 2D steady state probability with the flux on top in the form of arrows
    phase=0.0
    Ecouple=32.0
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            ...
            output_file_name = (target_dir + "Pss_" + "Ecouple_{0}_E0_{1}_E1_{2}_psi1_{3}_psi2_{4}_n1_{5}_n2_{6}" + "_big_.pdf")
...
def plot_prob_flux_grid(target_dir):#grid of contourplots of the 2D equilibrium probability with the flux on top in the form of arrows

    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            f,axarr=plt.subplots(1,7,sharex='all',sharey='all', figsize=(12,2))
            output_file_name = (target_dir + "Peq_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}" + "_.pdf")
...
def plot_condprob_grid(target_dir):#grid of contourplots of the conditional, steady state probability
    ...
    for psi_1 in psi1_array:
        for psi_2 in psi2_array:
            f,axarr=plt.subplots(3,7,sharex='all',sharey='all', figsize=(12,6))
            output_file_name = (target_dir + "Condprob_10_flux_grid_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}" + "_big_low.pdf")
...
def plot_rel_flux_Ecouple(target_dir):#plot of the relative flux (flux F1 divided by flux Fo) as a function of the coupling strength
    output_file_name = (target_dir + "relflux_Ecouple_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}" + "_.pdf")
...
def plot_pmf_space(target_dir):#plot of the pmf along some coordinate
    ...
    output_file_name2 = (target_dir + "pmf_y_cond_force_plot_" + "E0_{0}_E1_{1}_psi1_{2}_psi2_{3}_n1_{4}_n2_{5}" + "_.pdf")
...
#                     force_y[i] = psi_2*phi_array
                ...
                ## plot correlation
                for jj, phi in enumerate(phase_array):
                    ax1.plot(power_array[:,jj], barrier_height_x[:,jj], linestyle='-', color=plt.cm.cool(colorlist[jj]), marker='.', label=label_lst[jj]) #
                    ax2.plot(power_array[:,jj], barrier_height_y[:,jj], linestyle='-', color=plt.cm.cool(colorlist[jj]), marker='.', label=label_lst[jj])
