...
def launchpad_coupled(
    ...
    cdef:
        ...
        double E_0
        double[:, :] prob      = np.zeros((N, N))
        double[:, :] p_now     = np.zeros((N, N))
        double[:, :] p_last    = np.zeros((N, N))
        double[:, :, :] flux   = np.zeros((2, N, N))   # initalize cumulative flux as list to keep track of flux at every position
        double[:] positions    = np.linspace(0, (2*pi)-dx, N)
    ...
    for x1 in range(N):
        for x2 in range(N):
            prob[x1, x2] = exp(-beta*potential_coupled(positions[x1], positions[x2], Ax, Axy, Ay))*(Z**(-1.0))
            E += potential_coupled(0.0, x2*dx, Ax, Axy, Ay) * exp(-beta * potential_coupled(0.0, x2*dx, Ax, Axy, Ay))*dx*(Z**(-1.0))
    ...
    work, heat, p_sum = run_simulation_coupled(
        p_now, p_last, flux, dx, dt, cycles, period, m, gamma, beta, Ax, Axy, Ay, H, A, E_after_relax
    )
...
cdef double force1(
    ...
    ) nogil: #need the force to calculate work
    cdef double f1 = (0.5)*(
...
cdef void calc_flux(
    ...
    for i in prange(1, N-1, schedule='static'):
        ...
        for j in prange(1, N-1, schedule='static'):
            # first component
